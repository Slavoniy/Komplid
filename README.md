...
Для автоматического создания бэкапов базы данных и их загрузки в S3 (MinIO) используйте скрипт `scripts/backup_db.sh`.
Рекомендуется добавить его в `cron` на сервере (например, для запуска каждый день в 02:00):
```bash
0 2 * * * /opt/constructiondocs/scripts/backup_db.sh >> /var/log/db_backup.log 2>&1
```
...
- `/scripts`: Системные и bash скрипты (бэкапы).
- `/terraform`: Infrastructure as Code (манифесты для развертывания серверов).
